...
- 🔁 **Anti-doublons** : mémoire de 14 jours par groupe (`historique_offres_<groupe>.json`), avec normalisation des URLs (APEC/Indeed) et du code postal (comparaison stricte du département, pas une recherche de sous-chaîne) pour ne pas ré-analyser/valider à tort une offre
...
- 🔬 **Étape 2 — relecture critique** (Mistral, `mistral-small-latest`) : un second modèle, **d'un fournisseur différent**, relit l'analyse complète du premier et produit la version **finale** — il peut corriger le score, compléter des points manqués, ou confirmer. Le score initial et un résumé de l'ajustement (`✅ Confirmé` / `🔧 Ajusté`) sont conservés pour traçabilité
...
- 🏢 **Nom d'entreprise fiabilisé** : pour les offres France Travail, le nom structuré fourni par l'API est utilisé en priorité plutôt que de laisser l'IA le deviner du texte libre (réduit le risque d'hallucination)
...
- 🎯 **Quota de sécurité** (`MAX_ANALYSES_PAR_RUN = 15`) : plafonne le nombre d'analyses IA par run et par groupe, pour rester dans les limites des tiers gratuits Groq/Mistral
- ✍️ **Génération de candidature** : pour toute offre notée ≥ 8/10 (score final, après relecture), un message d'accroche LinkedIn et une ébauche de lettre de motivation sont générés automatiquement

### Sorties & notifications
- 📊 **Tableau de bord Excel** par groupe + Master fusionné : entreprise, poste, score final, score initial, ajustement collaboratif, verdict, message LinkedIn, lettre de motivation, colonnes **Statut**/**Notes perso** libres
...
2. Chaque groupe : interroge France Travail + scrape les 5 plateformes, filtre, analyse via Groq puis Mistral, met à jour son Excel et son historique
